import React, { useEffect, useState } from 'react'
import { AiOutlinePause } from 'react-icons/ai';
import { BsFillPlayFill } from 'react-icons/bs';
import './App.css';

const styles = {
  tune: 'tune h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 flex place-items-center snap-start'
}

const Tune = ({ tune }) => {
  return (
    <div className={styles.tune}>
      <div className=' m-auto w-3/4 rounded-lg p-3 bg-black bg-opacity-50'>
        <img className='rounded-lg' src={tune.image} alt="" />
        <div>
        <input id="minmax-range" type="range" min="0" max="10" value="0" class="w-full h-2 bg-gray-50 rounded-lg appearance-none cursor-pointer dark:bg-gray-700" />
        </div>
        <TuneDetails tune={tune} />
      </div>
    </div>
  )
}

const TuneDetails = ({tune}) => {
  const [playing, setPlaying] = useState(false)
  return (
    <div className='mt-2 p-6 rounded-lg bg-gradient-to-r from-slate-800 to-slate-900'>
      <div className='play-controls flex justify-center'>
        {playing ? (

          <AiOutlinePause className='h-12 w-12 rounded-full bg-white p-2' onClick={() => setPlaying(false)} />
        ) : (

        <BsFillPlayFill className='h-12 w-12 rounded-full bg-white p-2' onClick={() => setPlaying(true)} />
        )}
      </div>
      <div>
        <p className='text-white text-center font-semibold mt-2'>{tune.name}</p>
      </div>
      <div className="mt-6 flex justify-center items-center">
        <img className='h-12 w-12 rounded-full' src={tune.profile.image} />
        <p className='text-white font-bold ml-5'>{tune.profile.handle}</p>
      </div>
    </div>
  )
}

const Header = () => {
  return (
    <div className='header text-center p-4 drop-shadow-xl	 sticky top-0 z-30 bg-black flex justify-center items-center bg-opacity-10'>
      <div>
        <img className='h-12 w-12 rounded-lg' src="data:image/png;base64," alt="" />
      </div>
      <div className='ml-6'>
        <h1 className='text-white font-semibold'>Lens Tunes</h1>
      </div>
    </div>
  )
}

const App = () => {
  const tuneData = [
    {
      image: "https://i-d-images.vice.com/images/articles/meta/2014/10/31/covering-michael-jackson-the-art-behind-xscape-1414771441.jpg?crop=1xw:1xh;center,center",
      name: "A place with No Name",
      profile: {
        handle: "Michael Jackson",
        image: "https://i-d-images.vice.com/images/articles/meta/2014/10/31/covering-michael-jackson-the-art-behind-xscape-1414771441.jpg?crop=1xw:1xh;center,center"
      },
      stats: {
        comments: 140,
        mirrors: 120,
        likes: 33
      }
    },
    {
      image: "https://m.media-amazon.com/images/I/415MW56ZTWL.__AC_QL70_ML2_.jpg",
      name: "What you won't do for love",
      profile: {
        handle: "Bobby Caldwell",
        image: "https://m.media-amazon.com/images/I/415MW56ZTWL.__AC_QL70_ML2_.jpg",

      },
      stats: {
        comments: 140,
        mirrors: 120,
        likes: 33
      }
    },
    {
      image: "https://i.scdn.co/image/ab67616d0000b273f8560af696925c50080dcc35",
      name: "Just the Two of Us",
      profile: {
        handle: "Bill Withers",
        image: "https://i.scdn.co/image/ab67616d0000b273f8560af696925c50080dcc35",

      },
      stats: {
        comments: 140,
        mirrors: 120,
        likes: 33
      }
    },
  ]
  return (
    <div className='page h-max bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 max-h-screen overflow-y-scroll snap snap-y snap-mandatory '>
      <Header />
      {tuneData.map((tune) => (
        <Tune tune={tune} />
      ))}
    </div>
  )
}

export default App